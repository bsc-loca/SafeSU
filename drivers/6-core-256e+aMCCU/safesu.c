...
unsigned safesu_mccu_set_event_weigths(const unsigned int input,
    ...
    switch (input) {
        ...
        case 11:
            ...
            break;
        case 12:
            EVENT_WEIGTH_REG3 &= ~(0x000000FF);
            EVENT_WEIGTH_REG3 |= (weigth);
            break;
        case 13:
            EVENT_WEIGTH_REG3 &= ~(0x0000FF00);
            EVENT_WEIGTH_REG3 |= (weigth << 8);
            break;
        case 14:
            EVENT_WEIGTH_REG3 &= ~(0x00FF0000);
            EVENT_WEIGTH_REG3 |= (weigth << 16);
            break;
        case 15:
            EVENT_WEIGTH_REG3 &= ~(0xFF000000);
            EVENT_WEIGTH_REG3 |= (weigth << 24);
            break;
        case 16:
            EVENT_WEIGTH_REG4 &= ~(0x000000FF);
            EVENT_WEIGTH_REG4 |= (weigth);
            break;
        case 17:
            EVENT_WEIGTH_REG4 &= ~(0x0000FF00);
            EVENT_WEIGTH_REG4 |= (weigth << 8);
            break;
        case 18:
            EVENT_WEIGTH_REG4 &= ~(0x00FF0000);
            EVENT_WEIGTH_REG4 |= (weigth << 16);
            break;
        case 19:
            EVENT_WEIGTH_REG4 &= ~(0xFF000000);
            EVENT_WEIGTH_REG4 |= (weigth << 24);
            break;
    ...
    printf("EVENT_WEIGTH_REG3 = %u\n", EVENT_WEIGTH_REG3);
    printf("EVENT_WEIGTH_REG4 = %u\n", EVENT_WEIGTH_REG4);
#endif
    return (0);
}
...
void write_register_range (unsigned int entry, unsigned int exit, unsigned int value){
    ...
    for(i=entry;i<=exit;i=i+1){
        p=(volatile uint32_t *)(uintptr_t)(SAFESU_ADDR+(i*4));
        *p=value;
#ifdef __SAFESU_LIB_DEBUG__
        printf("address:%x \n",(SAFESU_ADDR+(i*4)));
        printf("value :%x \n",value);
...
struct report_s report_safesu (void){
    ...
    //Counters
    var=(volatile uint32_t *)(uintptr_t)(safesu_addr+BASE_COUNTERS*4);
    reader=*var;
#ifdef __UART__
    printf("Counters  *******: %d\n",N_COUNTERS);
    printf("SoC events  *******: %d\n",CROSSBAR_INPUTS);
...
#endif
    report.ev0 = reader;
    var=(volatile uint32_t *)(uintptr_t)(var+4);
    reader=*var;
#ifdef __UART__
    ...
    //RDC Watermarks
    var=(volatile uint32_t *)(uintptr_t)(safesu_addr+BASE_RDC_WATERMARK*4);
    reader=*var;
#ifdef __UART__
...
#endif
    var=(volatile uint32_t *)(uintptr_t)(safesu_addr+(BASE_RDC_WATERMARK+1)*4);
    reader=*var;
#ifdef __UART__
...
#endif
    p=(volatile uint32_t *)(uintptr_t)(SAFESU_ADDR+(entry*4));
    //get current value
    current_value=*p;
    //set reset bit
