#define REG_WIDTH 32
#define N_COUNTERS 32
#define N_CONF_REGS 1
#define OVERFLOW 1
#define MCCU 1
#define RDC 1
#define BASE_CFG 0
#define END_CFG 0
#define BASE_COUNTERS 1
#define END_COUNTERS 32
#define BASE_OVERFLOW_INTR 33
#define BASE_OVERFLOW_MASK 33
#define N_OVERFLOW_MASK_REGS 1
#define END_OVERFLOW_MASK 33
#define BASE_OVERFLOW_VECT 34
#define N_OVERFLOW_VECT_REGS 1
#define END_OVERFLOW_VECT 34
#define N_OVERFLOW_REGS 2
#define END_OVERFLOW_INTR 34
#define MCCU_WEIGTHS_WIDTH 8
#define MCCU_N_CORES 4
#define MCCU_N_EVENTS 2
#define BASE_MCCU_CFG 35
#define N_MCCU_CFG 1
#define END_MCCU_CFG 35
#define BASE_MCCU_LIMITS 36
#define N_MCCU_LIMITS 4
#define END_MCCU_LIMITS 39
#define BASE_MCCU_QUOTA 40
#define N_MCCU_QUOTA 4
#define END_MCCU_QUOTA 43
#define BASE_MCCU_WEIGTHS 44
#define N_MCCU_WEIGTHS 2
#define END_MCCU_WEIGTHS 45
#define N_MCCU_REGS 11
#define RDC_WEIGTHS_WIDTH 8
#define RDC_N_CORES 4
#define RDC_N_EVENTS 2
#define BASE_RDC_VECT 46
#define N_RDC_VECT_REGS 1
#define END_RDC_VECT 46
#define BASE_RDC_WEIGTHS 44
#define N_RDC_WEIGTHS 0
#define END_RDC_WEIGTHS 45
#define BASE_RDC_WATERMARK 47
#define N_RDC_WATERMARK 2
#define END_RDC_WATERMARK 48
#define N_RDC_REGS 3
#define CROSSBAR_INPUTS 32
#define BASE_CROSSBAR 49
#define N_CROSSBAR_CFG 5
#define END_CROSSBAR 53
#define N_CROSSBAR_REGS 5
#define TOTAL_NREGS 22
#define END_SAFESU (TOTAL_NREGS-1)

#define _SAFESU_REG_TYPE (volatile unsigned int * )
#define R2A (REG_WIDTH/8)
// SAFESU base address
#define _SAFESUREG (_SAFESU_REG_TYPE(SAFESU_ADDR))
// SAFESU counter base address
#define _SAFESU_COUNTERS (_SAFESU_REG_TYPE(SAFESU_ADDR + R2A * BASE_COUNTERS ))

// SAFESU crossbar base address
#define _SAFESU_CROSSBAR (_SAFESU_REG_TYPE(SAFESU_ADDR + R2A * BASE_CROSSBAR))

// SAFESU overflow interrupt register base address
#define _SAFESU_OVERFLOW (_SAFESU_REG_TYPE(SAFESU_ADDR + R2A * BASE_OVERFLOW_INTR))

#define SAFESUCFG0 (_SAFESUREG[0]) // SAFESU configuration register 0
#define CROSSBAR_REG0 (_SAFESU_CROSSBAR[0]) // Crossbar output register 0
#define CROSSBAR_REG1 (_SAFESU_CROSSBAR[1]) // Crossbar output register 1
#define CROSSBAR_REG2 (_SAFESU_CROSSBAR[2]) // Crossbar output register 2
#define CROSSBAR_REG3 (_SAFESU_CROSSBAR[3]) // Crossbar output register 3
#define CROSSBAR_REG4 (_SAFESU_CROSSBAR[4]) // Crossbar output register 4
#define CROSSBAR_REG5 (_SAFESU_CROSSBAR[5]) // Crossbar output register 5

// SAFESU overflow (I)nterrupt (E)nable register 
#define SAFESU_OVERLFOW_IE (_SAFESU_OVERFLOW[0])

// SAFESU overflow (I)nterrupt (V)ector register
#define SAFESU_OVERFLOW_IV (_SAFESU_OVERFLOW[1])

#define CROSSBAR_OUTPUT_0 (0)
#define CROSSBAR_OUTPUT_1 (1)
#define CROSSBAR_OUTPUT_2 (2)
#define CROSSBAR_OUTPUT_3 (3)
#define CROSSBAR_OUTPUT_4 (4)
#define CROSSBAR_OUTPUT_5 (5)
#define CROSSBAR_OUTPUT_6 (6)
#define CROSSBAR_OUTPUT_7 (7)
#define CROSSBAR_OUTPUT_8 (8)
#define CROSSBAR_OUTPUT_9 (9)
#define CROSSBAR_OUTPUT_10 (10U)
#define CROSSBAR_OUTPUT_11 (11u)
#define CROSSBAR_OUTPUT_12 (12u)
#define CROSSBAR_OUTPUT_13 (13u)
#define CROSSBAR_OUTPUT_14 (14u)
#define CROSSBAR_OUTPUT_15 (15u)
#define CROSSBAR_OUTPUT_16 (16u)
#define CROSSBAR_OUTPUT_17 (17u)
#define CROSSBAR_OUTPUT_18 (18u)
#define CROSSBAR_OUTPUT_19 (19u)
#define CROSSBAR_OUTPUT_20 (20u)
#define CROSSBAR_OUTPUT_21 (21u)
#define CROSSBAR_OUTPUT_22 (22u)
#define CROSSBAR_OUTPUT_23 (23u)
#define CROSSBAR_OUTPUT_24 (24u)
#define CROSSBAR_OUTPUT_25 (25u)
#define CROSSBAR_OUTPUT_26 (26u)
#define CROSSBAR_OUTPUT_27 (27u)
#define CROSSBAR_OUTPUT_28 (28u)
#define CROSSBAR_OUTPUT_29 (29u)
#define CROSSBAR_OUTPUT_30 (30u)
#define CROSSBAR_OUTPUT_31 (31u)
//#define CROSSBAR_OUTPUT_32 (32u)
//#define CROSSBAR_OUTPUT_33 (33u)
//#define CROSSBAR_OUTPUT_34 (34u)
//#define CROSSBAR_OUTPUT_35 (35u)
//#define CROSSBAR_OUTPUT_36 (36u)
//#define CROSSBAR_OUTPUT_37 (37u)
//#define CROSSBAR_OUTPUT_38 (38u)
//#define CROSSBAR_OUTPUT_39 (39u)
//#define CROSSBAR_OUTPUT_40 (40u)
//#define CROSSBAR_OUTPUT_41 (41u)
//#define CROSSBAR_OUTPUT_42 (42u)
//#define CROSSBAR_OUTPUT_43 (43u)
//#define CROSSBAR_OUTPUT_44 (44u)
//#define CROSSBAR_OUTPUT_45 (45u)
//#define CROSSBAR_OUTPUT_46 (46u)
//#define CROSSBAR_OUTPUT_47 (47u)
//#define CROSSBAR_OUTPUT_48 (48u)
//#define CROSSBAR_OUTPUT_49 (49u)
//#define CROSSBAR_OUTPUT_50 (50u)
//#define CROSSBAR_OUTPUT_51 (51u)
//#define CROSSBAR_OUTPUT_52 (52u)
//#define CROSSBAR_OUTPUT_53 (53u)
//#define CROSSBAR_OUTPUT_54 (54u)
//#define CROSSBAR_OUTPUT_55 (55u)
//#define CROSSBAR_OUTPUT_56 (56u)
//#define CROSSBAR_OUTPUT_57 (57u)
//#define CROSSBAR_OUTPUT_58 (58u)
//#define CROSSBAR_OUTPUT_59 (59u)
//#define CROSSBAR_OUTPUT_60 (60u)
//#define CROSSBAR_OUTPUT_61 (61u)
//#define CROSSBAR_OUTPUT_62 (62u)
//#define CROSSBAR_OUTPUT_63 (63u)

#define EVENT_0 (0u)
#define EVENT_1 (1u)
#define EVENT_2 (2u)
#define EVENT_3 (3u)
#define EVENT_4 (4u)
#define EVENT_5 (5u)
#define EVENT_6 (6u)
#define EVENT_7 (7u)
#define EVENT_8 (8u)
#define EVENT_9 (9u)
#define EVENT_10 (10u)
#define EVENT_11 (11u)
#define EVENT_12 (12u)
#define EVENT_13 (13u)
#define EVENT_14 (14u)
#define EVENT_15 (15u)
#define EVENT_16 (16u)
#define EVENT_17 (17u)
#define EVENT_18 (18u)
#define EVENT_19 (19u)
#define EVENT_20 (20u)
#define EVENT_21 (21u)
#define EVENT_22 (22u)
#define EVENT_23 (23u)
#define EVENT_24 (24u)
#define EVENT_25 (25u)
#define EVENT_26 (26u)
#define EVENT_27 (27u)
#define EVENT_28 (28u)
#define EVENT_29 (29u)
#define EVENT_30 (30u)
#define EVENT_31 (31u)
//#define EVENT_32 (32u)
//#define EVENT_33 (33u)
//#define EVENT_34 (34u)
//#define EVENT_35 (35u)
//#define EVENT_36 (36u)
//#define EVENT_37 (37u)
//#define EVENT_38 (38u)
//#define EVENT_39 (39u)
//#define EVENT_40 (40u)
//#define EVENT_41 (41u)
//#define EVENT_42 (42u)
//#define EVENT_43 (43u)
//#define EVENT_44 (44u)
//#define EVENT_45 (45u)
//#define EVENT_46 (46u)
//#define EVENT_47 (47u)
//#define EVENT_48 (48u)
//#define EVENT_49 (49u)
//#define EVENT_50 (50u)
//#define EVENT_51 (51u)
//#define EVENT_52 (52u)
//#define EVENT_53 (53u)
//#define EVENT_54 (54u)
//#define EVENT_55 (55u)
//#define EVENT_56 (56u)
//#define EVENT_57 (57u)
//#define EVENT_58 (58u)
//#define EVENT_59 (59u)
//#define EVENT_60 (60u)
//#define EVENT_61 (61u)
//#define EVENT_62 (62u)
//#define EVENT_63 (63u)
//#define EVENT_64 (64u)
//#define EVENT_65 (65u)
//#define EVENT_66 (66u)
//#define EVENT_67 (67u)
//#define EVENT_68 (68u)
//#define EVENT_69 (69u)
//#define EVENT_70 (70u)
//#define EVENT_71 (71u)
//#define EVENT_72 (72u)
//#define EVENT_73 (73u)
//#define EVENT_74 (74u)
//#define EVENT_75 (75u)
//#define EVENT_76 (76u)
//#define EVENT_77 (77u)
//#define EVENT_78 (78u)
//#define EVENT_79 (79u)
//#define EVENT_80 (80u)
//#define EVENT_81 (81u)
//#define EVENT_82 (82u)
//#define EVENT_83 (83u)
//#define EVENT_84 (84u)
//#define EVENT_85 (85u)
//#define EVENT_86 (86u)
//#define EVENT_87 (87u)
//#define EVENT_88 (88u)
//#define EVENT_89 (89u)
//#define EVENT_90 (90u)
//#define EVENT_91 (91u)
//#define EVENT_92 (92u)
//#define EVENT_93 (93u)
//#define EVENT_94 (94u)
//#define EVENT_95 (95u)
//#define EVENT_96 (96u)
//#define EVENT_97 (97u)
//#define EVENT_98 (98u)
//#define EVENT_99 (99u)
//#define EVENT_100 (100u)
//#define EVENT_101 (101u)
//#define EVENT_102 (102u)
//#define EVENT_103 (103u)
//#define EVENT_104 (104u)
//#define EVENT_105 (105u)
//#define EVENT_106 (106u)
//#define EVENT_107 (107u)
//#define EVENT_108 (108u)
//#define EVENT_109 (109u)
//#define EVENT_110 (110u)
//#define EVENT_111 (111u)
//#define EVENT_112 (112u)
//#define EVENT_113 (113u)
//#define EVENT_114 (114u)
//#define EVENT_115 (115u)
//#define EVENT_116 (116u)
//#define EVENT_117 (117u)
//#define EVENT_118 (118u)
//#define EVENT_119 (119u)
//#define EVENT_120 (120u)
//#define EVENT_121 (121u)
//#define EVENT_122 (122u)
//#define EVENT_123 (123u)
//#define EVENT_124 (124u)
//#define EVENT_125 (125u)
//#define EVENT_126 (126u)
//#define EVENT_127 (127u)
//#define EVENT_128 (128u)
//#define EVENT_129 (129u)
//#define EVENT_130 (130u)
//#define EVENT_131 (131u)
//#define EVENT_132 (132u)
//#define EVENT_133 (133u)
//#define EVENT_134 (134u)
//#define EVENT_135 (135u)
//#define EVENT_136 (136u)
//#define EVENT_137 (137u)
//#define EVENT_138 (138u)
//#define EVENT_139 (139u)
//#define EVENT_140 (140u)
//#define EVENT_141 (141u)
//#define EVENT_142 (142u)
//#define EVENT_143 (143u)
//#define EVENT_144 (144u)
//#define EVENT_145 (145u)
//#define EVENT_146 (146u)
//#define EVENT_147 (147u)
//#define EVENT_148 (148u)
//#define EVENT_149 (149u)
//#define EVENT_150 (150u)
//#define EVENT_151 (151u)
//#define EVENT_152 (152u)
//#define EVENT_153 (153u)
//#define EVENT_154 (154u)
//#define EVENT_155 (155u)
//#define EVENT_156 (156u)
//#define EVENT_157 (157u)
//#define EVENT_158 (158u)
//#define EVENT_159 (159u)
//#define EVENT_160 (160u)
//#define EVENT_161 (161u)
//#define EVENT_162 (162u)
//#define EVENT_163 (163u)
//#define EVENT_164 (164u)
//#define EVENT_165 (165u)
//#define EVENT_166 (166u)
//#define EVENT_167 (167u)
//#define EVENT_168 (168u)
//#define EVENT_169 (169u)
//#define EVENT_170 (170u)
//#define EVENT_171 (171u)
//#define EVENT_172 (172u)
//#define EVENT_173 (173u)
//#define EVENT_174 (174u)
//#define EVENT_175 (175u)
//#define EVENT_176 (176u)
//#define EVENT_177 (177u)
//#define EVENT_178 (178u)
//#define EVENT_179 (179u)
//#define EVENT_180 (180u)
//#define EVENT_181 (181u)
//#define EVENT_182 (182u)
//#define EVENT_183 (183u)
//#define EVENT_184 (184u)
//#define EVENT_185 (185u)
//#define EVENT_186 (186u)
//#define EVENT_187 (187u)
//#define EVENT_188 (188u)
//#define EVENT_189 (189u)
//#define EVENT_190 (190u)
//#define EVENT_191 (191u)
//#define EVENT_192 (192u)
//#define EVENT_193 (193u)
//#define EVENT_194 (194u)
//#define EVENT_195 (195u)
//#define EVENT_196 (196u)
//#define EVENT_197 (197u)
//#define EVENT_198 (198u)
//#define EVENT_199 (199u)
//#define EVENT_200 (200u)
//#define EVENT_201 (201u)
//#define EVENT_202 (202u)
//#define EVENT_203 (203u)
//#define EVENT_204 (204u)
//#define EVENT_205 (205u)
//#define EVENT_206 (206u)
//#define EVENT_207 (207u)
//#define EVENT_208 (208u)
//#define EVENT_209 (209u)
//#define EVENT_210 (210u)
//#define EVENT_211 (211u)
//#define EVENT_212 (212u)
//#define EVENT_213 (213u)
//#define EVENT_214 (214u)
//#define EVENT_215 (215u)
//#define EVENT_216 (216u)
//#define EVENT_217 (217u)
//#define EVENT_218 (218u)
//#define EVENT_219 (219u)
//#define EVENT_220 (220u)
//#define EVENT_221 (221u)
//#define EVENT_222 (222u)
//#define EVENT_223 (223u)
//#define EVENT_224 (224u)
//#define EVENT_225 (225u)
//#define EVENT_226 (226u)
//#define EVENT_227 (227u)
//#define EVENT_228 (228u)
//#define EVENT_229 (229u)
//#define EVENT_230 (230u)
//#define EVENT_231 (231u)
//#define EVENT_232 (232u)
//#define EVENT_233 (233u)
//#define EVENT_234 (234u)
//#define EVENT_235 (235u)
//#define EVENT_236 (236u)
//#define EVENT_237 (237u)
//#define EVENT_238 (238u)
//#define EVENT_239 (239u)
//#define EVENT_240 (240u)
//#define EVENT_241 (241u)
//#define EVENT_242 (242u)
//#define EVENT_243 (243u)
//#define EVENT_244 (244u)
//#define EVENT_245 (245u)
//#define EVENT_246 (246u)
//#define EVENT_247 (247u)
//#define EVENT_248 (248u)
//#define EVENT_249 (249u)
//#define EVENT_250 (250u)
//#define EVENT_251 (251u)
//#define EVENT_252 (252u)
//#define EVENT_253 (253u)
//#define EVENT_254 (254u)
//#define EVENT_255 (255u)

#define _SAFESU_MCCU_RDC (_SAFESU_REG_TYPE(SAFESU_ADDR + R2A * BASE_MCCU_CFG))
#define _SAFESU_MCCU_QUOTA (_SAFESU_REG_TYPE(SAFESU_ADDR +  R2A * BASE_MCCU_LIMITS))
#define _SAFESU_RDC_WATERMARKS (_SAFESU_REG_TYPE(SAFESU_ADDR + R2A * BASE_RDC_WATERMARK))

#define SAFESUCFG1 (_SAFESU_MCCU_RDC[0]) // SAFESU configuration register 1

#define SAFESU_QUOTA_SET0 (_SAFESU_MCCU_QUOTA[0]) // Quota set register 
#define SAFESU_QUOTA_SET1 (_SAFESU_MCCU_QUOTA[1]) // Quota set register 
#define SAFESU_QUOTA_SET2 (_SAFESU_MCCU_QUOTA[2]) // Quota set register 
#define SAFESU_QUOTA_SET3 (_SAFESU_MCCU_QUOTA[3]) // Quota set register 

#define SAFESU_QUOTA_REM0 (_SAFESU_MCCU_QUOTA[4]) // Quota current remaining for core 0
#define SAFESU_QUOTA_REM1 (_SAFESU_MCCU_QUOTA[5]) // Quota current remaining for core 1
#define SAFESU_QUOTA_REM2 (_SAFESU_MCCU_QUOTA[6]) // Quota current remaining for core 2
#define SAFESU_QUOTA_REM3 (_SAFESU_MCCU_QUOTA[7]) // Quota current remaining for core 3
#define EVENT_WEIGTH_REG0 (_SAFESU_MCCU_QUOTA[8]) // Event weight register 0 (input 0 to 3)
#define EVENT_WEIGTH_REG1 (_SAFESU_MCCU_QUOTA[9]) // Event weight register 1 (input 4 to 7)

#define _SAFESU_RDC_IV (_SAFESU_REG_TYPE(SAFESU_ADDR + BASE_RDC_VECT * R2A))
#define SAFESU_RDC_IV (_SAFESU_RDC_IV[0])
#define SAFESU_RDC_WATERMARK_REG0 (_SAFESU_RDC_WATERMARKS[0])
#define SAFESU_RDC_WATERMARK_REG1 (_SAFESU_RDC_WATERMARKS[1])
#define EV_WEIGTH_INPUT0 (0u)
#define EV_WEIGTH_INPUT1 (1u)
#define EV_WEIGTH_INPUT2 (2u)
#define EV_WEIGTH_INPUT3 (3u)
#define EV_WEIGTH_INPUT4 (4u)
#define EV_WEIGTH_INPUT5 (5u)
#define EV_WEIGTH_INPUT6 (6u)
#define EV_WEIGTH_INPUT7 (7u)
#define SAFESU_OVERFLOW_INT_INDEX (1u)